import numpy
from cipher_config import sbox, state_bits, key_bits, round_key_bits, sbox_bits, total_rounds, round_constants
...
def get_correlation_of_quasidifferential(characteristic, trail):
    ...
    for i in range(total_rounds):
        # No data-path constants in RECTANGLE (expanded-key model).
...
def get_average_w(differential_route, n, m):
    ...
    for i in range(total_rounds):
        for j in range(0, state_bits, sbox_bits):
            ...
            if pab == 0:
                raise ValueError(
                    f"Invalid differential transition at round {i}, "
                    f"S-box {j // sbox_bits}: a={d_aa}, b={d_bb}"
                )
...
def get_expanded_key_mask(k_trail):
    """All 64 bits of each independent round key, flattened round-major."""
    k_mask = [0] * key_bits
    for r, current_round_k_mask in enumerate(k_trail):
        for bit in range(round_key_bits):
            k_mask[r * round_key_bits + bit] = (current_round_k_mask >> bit) & 1
    return k_mask

def get_a_key_trail(q_trail):
    k_trail = []
    for i in range(total_rounds):
        # RECTANGLE: AddRoundKey -> SubColumn -> ShiftRow.
        # K_i takes the CURRENT S-box input mask m_a[i], not m_a[i+1].
        k_trail.append(q_trail[trail_number * i])
...
def get_cor_and_key_by_one_quasidifferential(characteristic, one_q_trail):
    ...
    expanded_k_trail = get_expanded_key_mask(one_k_trail)
    input_x = one_q_trail[0]
    for i in range(state_bits):
        expanded_k_trail.append((input_x >> i) & 0b1)

    return one_q_cor, expanded_k_trail
